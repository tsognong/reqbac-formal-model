...
public class BRelation<K, V> extends BSet<Pair<K, V>> {
	...
	/*
	 * @ requires true; ensures this.isEmpty();
	 */
	public BRelation() {
	}

	/*
	 * @ requires true; ensures this.equals(elems);
	 */
	...
	/*
	 * @ requires true; assignable this.*; ensures (\forall K k; (\forall V v;
	 * this.has(k, v) <==> (\exists int i; 0 <= i && i < pairs.length;
	...
	/*
	 * @ requires true; assignable this.*; ensures (\forall Pair<K,V> p; this.has(p)
	 * <==> s.contains(p));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures \result.int_size() == 1 &&
	 * \result.has(key, value);
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures (\forall V v; \result.has(v)
	 * <==> this.has(key, v));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures (\forall K key; keys.has(key);
	...
	/*
	 * requires true; assignable \nothing; ensures (\forall K key; (\forall V value;
	...
	/*
	 * @ requires true; assignable \nothing; ensures (\forall K key;
	 * \result.has(key) <==> this.has(key, value));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures (\forall V value;
	 * values.has(value); (\forall K key; \result.has(key) <==> this.has(key,
	 * value)));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures \result <==> (\exists V value;
	 * this.has(key, value));
	 */
	...
	/*
	 * @ requires true; assignable this.*; ensures this.has(pair);
	 */
	...
	/*
	 * @ requires true; assignable this.*; ensures this.has(new
	 * Pair<K,V>(key,value));
	 */
	...
	/*
	 * @ also requires true; assignable \nothing; ensures
	 * \result.equals(insert(pair.fst(), pair.snd()));
	 */
	...
	/*
	 * @ requires true; assignable this.*; ensures \result.has(new
	 * Pair<K,V>(key,value));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures \result <==> this.has(key,
	 * value);
	 */
	...
	/*
	 * @ also requires !(obj instanceof BSet); assignable \nothing; ensures \result
	 * <==> false; also requires obj instanceof BSet; assignable \nothing; ensures
	 * \result <==> (\forall Pair<K, V> pair; this.has(pair) <==> ((BSet)
	 * obj).has(pair));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures (\forall K key;
	 * \result.has(key) <==> this.isDefinedAt(key));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures (\forall V value;
	 * \result.has(value) <==> (\exists K key; this.has(key, value)));
	 */
	...
	/*
	 * @ requires true; assignable this.*; ensures ! \result.has(pair);
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures (\forall Pair<D, V> rp;
	 * \result.has(rp) <==> (\exists Pair<K, V> tp; this.has(tp); (\exists Pair<D,
	 * K> op; otherRel.has(op); op.snd().equals(tp.fst()) &&
	 * rp.snd().equals(tp.snd()) && tp.fst().equals(op.fst()))));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures (\forall Pair<K, V> pair;
	 * \result.has(pair) <==> this.has(pair) || otherRel.has(pair));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures (\forall Pair<K, V> e;
	 * \result.has(e) <==> (\exists int i; 0 <= 1 && i < sets.length;
	 * sets[i].has(e)));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures (\forall Pair<K, V> e;
	 * \result.has(e) <==> (\forall int i; 0 <= 1 && i < sets.length;
	 * sets[i].has(e)));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures
	 * \result.equals(this.associations());
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures (\forall Pair<K, BSet<V>> pair;
	 * \result.has(pair); this.isDefinedAt(pair.fst()) &&
	 * pair.snd().equals(this.elementImage(pair.fst())));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures
	 * \result.equals(this.domainElements());
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures
	 * \result.equals(this.rangeElements());
	 */
	 ...
	 * \result.domain().equals(dom.intersection(this.domain())) && (\forall V value;
	 * \result.range().has(value) <==> (\exists K k; dom.has(k); this.has(k,
	 * value)));
	 */
	 ...
	 * \result.range().equals(ran.intersection(this.range())) && (\forall K k;
	 * \result.domain().has(k) <==> (\exists V v; ran.has(v); this.has(k, v)));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures
	 * \result.equals(this.restrictDomainTo(this.domain().difference(domain)));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures
	 * \result.equals(restrictRangeTo(range().difference(range)));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures
	 * \result.equals(otherRel.compose(this));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures
	 * \result.equals(other.union(domainSubtraction(other.domain())));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures (\forall Pair<K, Pair<V, D>>
	 * pair; \result.has(pair) <==> (\exists Pair<K, V> tp; this.has(tp); (\exists
	 * Pair<K, D> op; other.has(op); tp.fst().equals(op.fst()) &&
	 * tp.fst().equals(pair.fst()) && pair.snd().equals(new Pair<V, D>(tp.snd(),
	 * op.snd())))));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures (\forall Pair<Pair<K, K1>,
	 * Pair<V, V1>> pair; \result.has(pair) <==> this.has(pair.fst()) &&
	 * other.has(pair.snd()));
	 */
	public /* @ pure */ <K1, V1> BRelation<Pair<K, K1>, Pair<V, V1>> directProd(BRelation<K1, V1> other) {
		...
		for (Pair<K, V> pair : this) {
			for (Pair<K1, V1> pair2 : other) {
				res.add(new Pair<K, K1>(pair.fst(), pair2.fst()), new Pair<V, V1>(pair.snd(), pair2.snd()));
			}
		}
		return res;
	}
	...
	/*
	 * @ requires true; assignable \nothing; ensures \result <==>
	 * dom.equals(domain()) && range().isSubset(ran);
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures \result <==>
	 * domain().isSubset(dom) && ran.equals(range());
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures \result <==> isTotal(dom, ran)
	 * && isSurjective(dom, ran);
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures \result <==> isaFunction() &&
	 * isRelation(dom, ran);
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures \result <==>
	 * domain().isSubset(dom) && range().isSubset(ran);
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures \result <==> isaFunction() &&
	 * isTotal(dom, ran) && range().isSubset(ran);
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures \result <==> isaFunction() &&
	 * inverse().isaFunction() && isRelation(dom, ran);
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures \result <==>
	 * isPartialInjection(dom, ran) && isTotal(dom, ran);
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures \result <==>
	 * isTotalFunction(dom, ran) && isPartialSurjection(dom, ran);
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures \result <==>
	 * isPartialInjection(dom, ran) && isPartialSurjection(dom, ran);
	 */
	...
	/*
	 * @ public normal_behavior requires this.isaFunction(); assignable \nothing;
	 * ensures \result.equals(this.elementImage(key).choose()); also public
	 * exceptional_behavior requires !this.isaFunction(); assignable \nothing;
	...
	/*
	 * @ also requires true; assignable \nothing; ensures (\forall BSet<Pair<K, V>>
	 * s; \result.has(s) <==> s.isSubset(this));
	 */
	...
	/*
	 * @ also requires true; assignable \nothing; ensures (\forall BSet<Pair<K, V>>
	 * s; \result.has(s) <==> s.isSubset(this) && !s.isEmpty());
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures \result <==> (\forall Pair<K,
	 * V> e; c.contains(e); this.has(e));
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures \result <==>
	 * this.isSubset(other) && !this.equals(other);
	 */
	...
	/*
	 * @ requires true; assignable \nothing; ensures (\forall Pair<K, V> e;
	 * \result.has(e) <==> domain.has(e.fst()) && range.has(e.snd()));
	 */
